...
class ResUserViettelBranch(models.Model):
    _name = "res.users.viettel.branch"
    _description = "User Viettel Branch"

    name = fields.Char('Name')
    branch_id = fields.Many2one('einvoice.viettel.branch', string='Viettel branch')
    user_id = fields.Many2one('res.users', string='User')
    is_update_einvoice = fields.Boolean('Created in Viettel', default=False)


class ResUser(models.Model):
    _inherit = "res.users"

    seller_code = fields.Char('User Code')

    def is_existed_branch(self, branch_id):
        self.ensure_one()
        user_branch = self.env['res.users.viettel.branch'].sudo().search(
            [('user_id', '=', self.id), ('branch_id', '=', branch_id)], limit=1)
        if len(user_branch.ids) > 0:
            return user_branch
        else:
            new = self.env['res.users.viettel.branch'].sudo().create({
                'branch_id': branch_id,
                'user_id': self.id,
    ...
    def update_to_einvoice(self, branch):
        ...
        sellerCode = self.seller_code or '11111111'
    ...
    def create_seller_branch(self):
        branches = self.env['einvoice.viettel.branch'].sudo().search([])

...
class ExportVATInInvoiceWizard(models.TransientModel):
    _name = 'export.vat.in.invoice.wizard'
    _inherit = 'export.vat.invoice.common'
    _description = 'Export Vat In Invoice Wizard'

    def _get_report_name(self):
        return _('Vat In Invoice')

    def get_lines(self):
        in_lines = self.env['account.vat.invoice.line'].search_read(domain=[
        ...
        ], fields=['invoice_id', 'price_subtotal', 'vat_amount'])
    ...
    def _inject_report_into_xlsx_sheet(self, workbook, sheet):
        ...
        # Form Name
        template_name = "Mẫu số 01: 01 -2/GTGT(Ban hành kèm theo thông tư số 119/2014/TT-BTC ngày 25/8/2014 của Bộ tài chính)"
        write_with_merge(sheet, 0, 6, 1, 7, template_name, form_format)

        # Report title
        report_title = "BẢNG KÊ HÓA ĐƠN, CHỨNG TỪ CỦA HÀNG HÓA, DỊCH VỤ MUA VÀO"
        ...
        row += 1
        title_cell_str = _("1. Hàng hóa, dịch vụ dùng cho SXKD chịu thuế được khấu trừ")
        ...
        row += 1
        title_cell_str = _("2. Hàng hóa, dịch vụ dùng riêng cho SXKD không chịu thuế GTGT")
        write_with_merge(sheet, row, col, row, col + 7, title_cell_str, cell_title_format)
        row += 1
        title_cell_str = _("Tổng")
        write_with_merge(sheet, row, col, row, col + 4, title_cell_str, cell_title_format)
        val_cell = 0
        write_cell(sheet, row, 5, val_cell, cell_title_format)
        val_cell = 0
        write_cell(sheet, row, 6, val_cell, cell_title_format)
        val_cell = ''
        write_cell(sheet, row, 7, val_cell, cell_title_format)

        row += 1
        title_cell_str = _("3. Hàng hóa, dịch vụ dùng chung cho SXKD chịu thuế và không chịu thuế được khấu trừ thuế")
        write_with_merge(sheet, row, col, row, col + 7, title_cell_str, cell_title_format)
        row += 1
        title_cell_str = _("Tổng")
        write_with_merge(sheet, row, col, row, col + 4, title_cell_str, cell_title_format)
        ...
        row += 1
        title_cell_str = _("4. Hàng hóa, dịch vụ dùng cho dự án đầu tư đủ điều kiện được khấu trừ thuế")
        write_with_merge(sheet, row, col, row, col + 7, title_cell_str, cell_title_format)
        row += 1
        title_cell_str = _("Tổng")
        write_with_merge(sheet, row, col, row, col + 4, title_cell_str, cell_title_format)
        ...
        row += 1
        row += 1
        title_cell_str = _("Tổng giá trị HHDV mua vào phục vụ SXKD được khấu trừ thuế GTGT")
        ...
        row += 1
        title_cell_str = _("Tổng số thuế GTGT của hàng hóa, dịch vụ mua vào")
        write_with_merge(sheet, row, 0, row, 2, title_cell_str, cell_title_format)

...
class ReportPartnerLedger(models.AbstractModel):
    ...
    filter_account_type = [
        ...
        {'id': 'other_141101', 'name': _lt('141101- Tạm ứng - Công tác phí'), 'selected': False},
        {'id': 'other_141201', 'name': _lt('141201 - Tạm ứng - Lương'), 'selected': False},
        {'id': 'other_141301', 'name': _lt('141301 - Tạm ứng - Thi công XDCB, sửa chữa, mua sắm'), 'selected': False},
    ]
    ...
    def generate_option_account_type(self, options):
        ...
        for account_type_option in options.get('account_type', []):
            if account_type_option['selected']:
                if account_type_option['id'] == 'other_141101':
                    if have_or:
                        domain.insert(0, '|')
                    domain.append(('account_id.code', '=', '141101'))
                    have_or = True
                if account_type_option['id'] == 'other_141201':
                    if have_or:
                        domain.insert(0, '|')
                    domain.append(('account_id.code', '=', '141201'))
                    have_or = True
                if account_type_option['id'] == 'other_141301':
                    if have_or:
                        domain.insert(0, '|')
                    domain.append(('account_id.code', '=', '141301'))
                    have_or = True
            print(domain)
        return domain
    ...
    def _get_query_amls(self, options, expanded_partner=None, offset=None, limit=None):
        ...
        query = '''
                SELECT
                    ...
                    account_move_line.offset_account        AS offset_account

...
class AccountChartOfAccountReport(models.AbstractModel):
    ...
    filter_offset_account = True

    @api.model
    def _get_filter_offset_accounts(self):
        current_company = self.env['res.company'].browse(self._context.get('allowed_company_ids')).ids
        return self.env['account.account'].search([
            ('company_id', 'in', current_company),
        ])

    @api.model
    def _init_filter_offset_account(self, options, previous_options=None):
        if self.filter_offset_account is None:
            return

        if previous_options and previous_options.get('offset_accounts'):
            offset_account_map = dict(
                (opt['id'], opt['selected']) for opt in previous_options['offset_accounts'] if 'selected' in opt
            )
        else:
            offset_account_map = {}
        options['offset_accounts'] = []

        for record in self._get_filter_offset_accounts():
            options['offset_accounts'].append({
                'id': record.id,
                'name': record.code,
                'offset_account': record.display_name,
                'selected': bool(offset_account_map.get(record.id)),
            })
        return options


    def _get_options_offset_accounts(self, options):
        return [
            offset_account for offset_account in options.get('offset_accounts', []) if offset_account['selected']
        ]

    def _get_options_offset_accounts_domain(self, options):
        selected_offset_accounts = self._get_options_offset_accounts(options)
        return selected_offset_accounts and [('id', 'in', [record['id'] for record in selected_offset_accounts])] or []

    def get_report_information(self, options):
        options = self._get_options(options)
        if options.get('offset_accounts'):
            current_company = self.env.company.name
            offset_accounts_selected = set(
                offset_account['id'] for offset_account in options['offset_accounts'] if offset_account.get('selected'))
            for record in self.env['account.move.line'].search([
                ('company_id', '=', current_company),
                ('parent_state', '=', 'posted'),
                ('offset_account', '!=', '')
            ]):
                if offset_accounts_selected and offset_accounts_selected in record.offset_account_ids.ids:
                    options['id_offset_account'] = record.offset_account
                    break

        info = super(AccountChartOfAccountReport, self).get_report_information(options)
        return info

    @api.model
    def _get_options(self, previous_options=None):
        options = super(AccountChartOfAccountReport, self)._get_options(previous_options=previous_options)
        if self.filter_offset_account:
            self._init_filter_offset_account(options, previous_options=previous_options)
        return options
    ...
    def export_detail(self, options, response=None):
        ...
        offset_accounts = options['offset_accounts']
        ...
        if offset_accounts and any(offset_account.get('selected', False) for offset_account in offset_accounts):
            domain += [
                ('offset_account', '=', [offset_account['offset_account'] for offset_account in offset_accounts if offset_account.get('selected', False)])]
        ...
        for y in range(0, len(lines) - 1):
            ...
            for journal_item in journal_items:
                ...
                sheet.write(y_offset, 3, journal_item.offset_account, level_3_style)
                ...
                if journal_item.move_id.move_type == 'in_invoice':
                    ...
                    elif journal_item.partner_id and journal_item.partner_id.company_type == 'person':

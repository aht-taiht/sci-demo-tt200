...
var SwitchCompanyMenu = Widget.extend({
    ...
    willStart: function () {
        ...
        if (this.all_companies) {
            this.current_company = session.user_companies.current_company[0];
            this.current_company_name = session.user_companies.current_company[1];
        }
...
//        $(ev.currentTarget).attr('aria-pressed', 'true');
    ...
    _onToggleAllCompanyClick: function (ev) {
        ...
        if (dropdownItem.find('.fa-square-o').length) {
            dropdownItem.find('.fa-square-o').removeClass('fa-square-o').addClass('fa-check-square');
            var company_ids = [];
            var user_companies = this.user_companies;
            for (var index in user_companies){
                company_ids.push(user_companies[index][0])
            }
            allowed_company_ids = company_ids;
//            session.setMultiCompanies(company_ids);
            session.setCompanies(company_ids, allowed_company_ids);
        } else {

...
class PurchaseOrderWoodXlsx(models.AbstractModel):
    _name = "report.soa_sale_management.sale_order_wood_xlsx"
    ...
    def write_report_name_to_sheet(self, sheet, x_offset, y_offset, style_fmt):
        ...
        write_with_colspan(sheet, x_offset, y_offset, 'PROFORMA INVOICE', 6, style_fmt['report_title_format'])
    ...
    def write_general_info_to_sheet(self, sheet, order, x_offset, y_offset, style_fmt):
        ...
        write_with_colspan(sheet, curr_x + 1, curr_y, order.date_order.strftime("%m-%d-%Y"), colspan=2, style=style_fmt['normal_info_format'])
        curr_y += 1
        write_with_colspan(sheet, curr_x, curr_y, 'Payment Term:', colspan=1, style=style_fmt['title_info_format'])
        write_with_colspan(sheet, curr_x + 1, curr_y, order.payment_term_id.name or '', colspan=2, style=style_fmt['normal_info_format'])
        curr_y += 1
        write_with_colspan(sheet, curr_x, curr_y, 'Delivery Term:', colspan=1, style=style_fmt['title_info_format'])
        write_with_colspan(sheet, curr_x + 1, curr_y, order.delivery_term or '', colspan=2, style=style_fmt['normal_info_format'])
        curr_y += 1
        write_with_colspan(sheet, curr_x, curr_y, 'Shipment Date:', colspan=1, style=style_fmt['title_info_format'])
        edt_order = order.etd_order and order.etd_order.strftime("%m-%d-%Y") or ''
    ...
    def write_note_info_to_sheet(self, sheet, order, x_offset, y_offset, style_fmt):
        ...
        y_curr += 1
        info_list = [
            ('Port of Loading:', order.port_of_loading or ''),
            ('Port of Discharge:', order.port_of_discharge or ''),
            ('Country of Origin:', order.country_of_origin or ''),
        ...
        write_with_colspan(sheet, x_curr + 1, y_curr, order.receiver_note or '', colspan=1, style=style_fmt['normal_info_format'])

...
class AccountInvoiceSourcingXlsx(models.AbstractModel):
    _name = "report.soa_sale_management.account_invoice_sourcing_xlsx"
    _inherit = "report.report_xlsx.abstract"
    _description = "Account Invoice Sourcing Report"
    ...
    def generate_xlsx_report(self, workbook, data, orders):
        ...
        for obj in orders:
            ...
            sale_order = obj.invoice_line_ids.sale_line_ids.order_id and obj.invoice_line_ids.sale_line_ids.order_id[-1]
            ...
            x_offset = 0
            y_offset = 20
            self.write_logicstic_info_to_sheet(sheet, sale_order, x_offset, y_offset, style_fmt)
            x_offset = 0
            y_offset = 29
            self.write_product_info_to_sheet(sheet, obj, x_offset, y_offset, style_fmt)
            x_offset = 0
            y_offset += len(obj.invoice_line_ids) + 2  # Header and Subheader
    ...
    def write_report_name_to_sheet(self, sheet, x_offset, y_offset, style_fmt):
        ...
        write_with_colspan(sheet, x_offset, y_offset, 'COMMERCIAL INVOICE', 32, style_fmt['title_format'])
    ...
    def write_general_info_to_sheet(self, sheet, order, x_offset, y_offset, style_fmt):
        ...
        seller_contact_info = f"Contact person: {seller_contact and seller_contact.name or ''}\nEmail: {seller_contact and seller_contact[-1].email or ''}\nTel: {seller_contact and seller_contact[-1].phone or ''}"
        curr_y += 1
    ...
    def write_logicstic_info_to_sheet(self, sheet, order, x_offset, y_offset, style_fmt):
        """
        params: order is a sale.order object
        """
        curr_x = x_offset
        curr_y = y_offset
        left_fields = [
    ...
    def write_subtotal_info_to_sheet(self, sheet, order, x_offset, y_offset, style_fmt):
        ...
        deposit_amount = order._get_paid_amount()
        subtotal_list.append(('DEPOSIT', order.currency_id.format(deposit_amount)))
    ...
    def write_note_info_to_sheet(self, sheet, order, x_offset, y_offset, style_fmt):
        note_list = [
            ...
            ('bold', "*Packing dimensions, weight, and loadibility are binding by this contract. No short-shipment nor overloading will be accepted, unless agreed upon, at the seller's request, and by written"),

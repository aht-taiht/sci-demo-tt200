...
class PurchaseOrderSourcingXlsx(models.AbstractModel):
    _name = "report.soa_sale_management.purchase_order_sourcing_xlsx"
    _inherit = "report.report_xlsx.abstract"
    _description = "Purchase Record Report"

    def generate_xlsx_report(self, workbook, data, orders):
        if not orders:
            raise ValidationError(_('Purchase Order Record is not found !'))
    ...
    def write_report_name_to_sheet(self, sheet, x_offset, y_offset, style_fmt):
        ...
        write_with_colspan(sheet, x_offset, y_offset, 'PURCHASE ORDER', 32, style_fmt['title_format'])
    ...
    def write_general_info_to_sheet(self, sheet, order, x_offset, y_offset, style_fmt):
        ...
        seller_contact = order.partner_id.child_ids.filtered(lambda x: x.type == 'contact')
        seller_contact_info = f"Contact person: {seller_contact.name or ''}\nEmail: {seller_contact.email or ''}\nTel: {seller_contact.phone or ''}"
        curr_y += 1
        ...
        buyer_name = order.company_id.name
        buyer_street, buyer_street2 = order.company_id.street or '', order.company_id.street2 or ''
        buyer_street_info = f"{buyer_street} {buyer_street2 and new_line or ''}{buyer_street2 or ''}"
        buyer_contact_info = f"Contact person: {order.user_id.name or ''}\nEmail: {order.user_id.email or ''}\nTel: {order.user_id.phone or ''}"
        curr_y += 1
    ...
    def write_logicstic_info_to_sheet(self, sheet, order, x_offset, y_offset, style_fmt):
        ...
        left_fields = [
            ...
            ('CBM:', 'report_cbm'),
            ('ETD:', 'purchase_etd.strftime("%m-%d-%Y")'),

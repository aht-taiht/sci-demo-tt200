...
class CashFlowReportWizard(models.TransientModel):
    _name = 'account.cashflow.report.wizard'
    _inherit = 'budget.combined.common'
    _description = 'CashFlow Report Wizard'

    company_ids = fields.Many2many('res.company', string='Company')
    report_type = fields.Selection([
        ('bu', 'Group by BU'),
        ('project', 'Detailed by Project'),
        ('reference', 'Detailed by Income/Expense'),
    ], string="Report Type", required=1)

    @api.model
    def default_get(self, fields_list):
        res = super(CashFlowReportWizard, self).default_get(fields_list)
    ...
    def action_export_to_xlsx(self):
        ...
        return {
            ...
            'url': '/report/download/xlsx/%s/%s/%s' % (
                'CashFlowReport', self._name, self.id)
        }
    ...
    # def _get_journal_initial_balance(self, date):
    ...
    #         -- JOIN account_account aa on journal.default_account_id = aa.id
    #         WHERE aml.parent_state = 'posted' AND aml.date < '{date}' AND journal.id in {journal_ids} AND aml.account_id = journal.default_account_id
    #         GROUP BY journal.id
    #     """.format(date=date, journal_ids=tuple(journal_ids.ids))
    ...
    # def _get_journal_end_balance(self, date):
    ...
    #         JOIN account_journal journal ON journal.id = aml.journal_id
    #         -- JOIN account_account aa on journal.default_account_id = aa.id
    #         WHERE aml.parent_state = 'posted' AND aml.date < '{end_date}' AND journal.id in {journal_ids} AND aml.account_id = journal.default_account_id
    #         GROUP BY journal.id
    #     """.format(end_date=end_date, journal_ids=tuple(journal_ids.ids))
    #     self._cr.execute(query)
    #     values = self._cr.fetchall()
    #     result = 0
    #     for value in values:
    ...
    def _get_cashflow_lines_project(self, report_lines, date, project_id):
        ...
        def _data_project_in_month(schedule_date, analytic_account_id):
            if schedule_date >= date and schedule_date < end_date and analytic_account_id == project_id:
                return True
            return False
    ...
    def _get_cashflow_lines_project_ref(self, report_lines, date, project_id):
        reportObj = self.env['account.cashflow.report']
        if date:
            end_date = date + relativedelta(months=1)
        today = fields.Date.context_today(self)
        ...
        def _data_project_ref_in_month(schedule_date, analytic_account_id):
            if schedule_date >= date and schedule_date < end_date and analytic_account_id == project_id:
                return True
            return False

        def _data_project_ref(analytic_account_id):
            if analytic_account_id == project_id:
                return True
            return False

        list_ref = []
        ref_values = {}
        false_ref_value = {'actual_in': 0, 'actual_out': 0, 'plan_in': 0, 'plan_out': 0}

        all_lines = list(filter(lambda d: _data_project_ref(d['analytic_account_id'][0]), report_lines))
        for line in all_lines:
            if line['ref'] and line['ref'] not in list_ref:
                list_ref.append(line['ref'])
                ref_values.update({line['ref']: {'actual_in': 0, 'actual_out': 0, 'plan_in': 0, 'plan_out': 0}})
            if not line['ref']:
                ref_values.update({'None': {'actual_in': 0, 'actual_out': 0, 'plan_in': 0, 'plan_out': 0}})

        if not date:
            return ref_values

        lines = list(filter(lambda d: _data_project_ref_in_month(
            d['schedule_date'], d['analytic_account_id'][0]), report_lines))

        for line in lines:
            schedule_date_month = line['schedule_date'].month
            schedule_date_year = line['schedule_date'].year
            line_ref = line['ref'] if line['ref'] else 'None'
            if (schedule_date_month >= this_month and schedule_date_year == this_year) or (schedule_date_year > this_year):
                # Plan
                if line['amount_financial_currency'] < 0:
                    ref_values[line_ref]['plan_out'] += line['amount_financial_currency']
                else:
                    ref_values[line_ref]['plan_in'] += line['amount_financial_currency']
            else:
                # Actual
                if line['actual_amount_financial_currency'] < 0:
                    ref_values[line_ref]['actual_out'] += line['actual_amount_financial_currency']
                else:
                    ref_values[line_ref]['actual_in'] += line['actual_amount_financial_currency']
    ...
    def _get_lines(self):
        ...
        projects = {}
        projects_name = {}
        if self.report_type in ['project', 'reference']:
    ...
    def _inject_report_into_xlsx_sheet(self, workbook, sheet):
        ...
        for bu in analytic_plan_ids:
            x_offset = -1
            total_value = {
                'total_actual_in': 0, 'total_actual_out': 0, 'total_received_margin': 0,
                'total_plan_in': 0, 'total_plan_out': 0, 'total_tobe_margin': 0, 'total_total_margin': 0,
            }
            if projects and bu.id in projects:
                total_project_value = {}
                for project in projects[bu.id]:
                    total_project_value.update({project: {
                        'total_actual_in': 0, 'total_actual_out': 0,
                        'total_plan_in': 0, 'total_plan_out': 0
                    }})
                ...
                if isinstance(key, date):
                    ...
                    if projects and bu.id in projects:
                        for project in projects[bu.id]:
                            ...
                            if self.report_type == 'reference':
                                ref_lines = self._get_cashflow_lines_project_ref(report_lines, key, project)
                                for ref, value in ref_lines.items():
                                    if project not in total_project_ref['actual_in']:
                                        total_project_ref['actual_in'].update({project: {}})
                                    if ref not in total_project_ref['actual_in'][project]:
                                        total_project_ref['actual_in'][project].update({ref: 0})
                                    total_project_ref['actual_in'][project][ref] += value['actual_in']
                    if projects and bu.id in projects:
                        for project in projects[bu.id]:
                            project_value = self._get_cashflow_lines_project(report_lines, key, project)
                            ...
                            if self.report_type == 'reference':
                                ref_lines = self._get_cashflow_lines_project_ref(report_lines, key, project)
                                for ref, value in ref_lines.items():
                                    if project not in total_project_ref['actual_out']:
                                        total_project_ref['actual_out'].update({project: {}})
                                    if ref not in total_project_ref['actual_out'][project]:
                                        total_project_ref['actual_out'][project].update({ref: 0})
                                    total_project_ref['actual_out'][project][ref] += value['actual_out']
                    if projects and bu.id in projects:
                        for project in projects[bu.id]:
                            project_value = self._get_cashflow_lines_project(report_lines, key, project)
                            ...
                            if self.report_type == 'reference':
                                ref_lines = self._get_cashflow_lines_project_ref(report_lines, key, project)
                                for ref, value in ref_lines.items():
                                    if project not in total_project_ref['plan_in']:
                                        total_project_ref['plan_in'].update({project: {}})
                                    if ref not in total_project_ref['plan_in'][project]:
                                        total_project_ref['plan_in'][project].update({ref: 0})
                                    total_project_ref['plan_in'][project][ref] += value['plan_in']
                    if projects and bu.id in projects:
                        for project in projects[bu.id]:
                            project_value = self._get_cashflow_lines_project(report_lines, key, project)
                            ...
                            if self.report_type == 'reference':
                                ref_lines = self._get_cashflow_lines_project_ref(report_lines, key, project)
                                for ref, value in ref_lines.items():
                                    if project not in total_project_ref['plan_out']:
                                        total_project_ref['plan_out'].update({project: {}})
                                    if ref not in total_project_ref['plan_out'][project]:
                                        total_project_ref['plan_out'][project].update({ref: 0})
                                    total_project_ref['plan_out'][project][ref] += value['plan_out']
            ...
            for key in column_keys:
                ...
                if key == 'bu':
                    ...
                    if projects and bu.id in projects:
                        for project in projects[bu.id]:
                            if total_project_value[project]['total_actual_in'] != 0:
                                write_with_colspan(sheet, x_offset, y_offset_tmp,
                                                   projects_name[project], 1, cf_project_style)
                                y_offset_tmp += 1
                            if self.report_type == 'reference':
                                ref_lines = self._get_cashflow_lines_project_ref(report_lines, False, project)
                                for ref, value in ref_lines.items():
                                    if total_project_ref['actual_in'][project][ref] != 0:
                                        write_with_colspan(sheet, x_offset, y_offset_tmp, ref, 1, cf_ref_style)
                                        y_offset_tmp += 1
                    ...
                    if projects and bu.id in projects:
                        for project in projects[bu.id]:
                            if total_project_value[project]['total_actual_out'] != 0:
                                write_with_colspan(sheet, x_offset, y_offset_tmp,
                                                   projects_name[project], 1, cf_project_style)
                                y_offset_tmp += 1
                            if self.report_type == 'reference':
                                ref_lines = self._get_cashflow_lines_project_ref(report_lines, False, project)
                                for ref, value in ref_lines.items():
                                    if total_project_ref['actual_out'][project][ref] != 0:
                                        write_with_colspan(sheet, x_offset, y_offset_tmp, ref, 1, cf_ref_style)
                                        y_offset_tmp += 1
                    ...
                    if projects and bu.id in projects:
                        for project in projects[bu.id]:
                            if total_project_value[project]['total_plan_in'] != 0:
                                write_with_colspan(sheet, x_offset, y_offset_tmp,
                                                   projects_name[project], 1, cf_project_style)
                                y_offset_tmp += 1
                            if self.report_type == 'reference':
                                ref_lines = self._get_cashflow_lines_project_ref(report_lines, False, project)
                                for ref, value in ref_lines.items():
                                    if total_project_ref['plan_in'][project][ref] != 0:
                                        write_with_colspan(sheet, x_offset, y_offset_tmp, ref, 1, cf_ref_style)
                                        y_offset_tmp += 1
                    ...
                    if projects and bu.id in projects:
                        for project in projects[bu.id]:
                            if total_project_value[project]['total_plan_out'] != 0:
                                write_with_colspan(sheet, x_offset, y_offset_tmp,
                                                   projects_name[project], 1, cf_project_style)
                                y_offset_tmp += 1
                            if self.report_type == 'reference':
                                ref_lines = self._get_cashflow_lines_project_ref(report_lines, False, project)
                                for ref, value in ref_lines.items():
                                    if total_project_ref['plan_out'][project][ref] != 0:
                                        write_with_colspan(sheet, x_offset, y_offset_tmp, ref, 1, cf_ref_style)
                                        y_offset_tmp += 1
                    ...
                    write_with_colspan(sheet, x_offset, y_offset_tmp,
                                       cell_values['actual_in'] if cell_values['actual_in'] != 0 else '-', 1, cell_style)
                    y_offset_tmp += 1
                    if projects and bu.id in projects:
                        for project in projects[bu.id]:
                            project_value = self._get_cashflow_lines_project(report_lines, key, project)
                            # total_project_value[project]['total_actual_in'] += project_value['actual_in']
                            if total_project_value[project]['total_actual_in'] != 0:
                                write_with_colspan(sheet, x_offset, y_offset_tmp, project_value['actual_in']
                                                   if project_value['actual_in'] != 0 else '-', 1, cell_style)
                                y_offset_tmp += 1
                            if self.report_type == 'reference':
                                ref_lines = self._get_cashflow_lines_project_ref(report_lines, key, project)
                                for ref, value in ref_lines.items():
                                    if total_project_ref['actual_in'][project][ref] != 0:
                                        write_with_colspan(
                                            sheet, x_offset, y_offset_tmp, value['actual_in'] if value['actual_in'] != 0 else '-', 1, cell_style)
                                        y_offset_tmp += 1
                    write_with_colspan(sheet, x_offset, y_offset_tmp,
                                       cell_values['actual_out'] if cell_values['actual_out'] != 0 else '-', 1, cell_style)
                    y_offset_tmp += 1
                    if projects and bu.id in projects:
                        for project in projects[bu.id]:
                            project_value = self._get_cashflow_lines_project(report_lines, key, project)
                            # total_project_value[project]['total_actual_out'] += project_value['actual_out']
                            if total_project_value[project]['total_actual_out'] != 0:
                                write_with_colspan(sheet, x_offset, y_offset_tmp, project_value['actual_out']
                                                   if project_value['actual_out'] != 0 else '-', 1, cell_style)
                                y_offset_tmp += 1
                            if self.report_type == 'reference':
                                ref_lines = self._get_cashflow_lines_project_ref(report_lines, key, project)
                                for ref, value in ref_lines.items():
                                    if total_project_ref['actual_out'][project][ref] != 0:
                                        write_with_colspan(sheet, x_offset, y_offset_tmp, value['actual_out']
                                                           if value['actual_out'] != 0 else '-', 1, cell_style)
                                        y_offset_tmp += 1
                    ...
                    y_offset_tmp += 1
                    write_with_colspan(sheet, x_offset, y_offset_tmp,
                                       cell_values['plan_in'] if cell_values['plan_in'] != 0 else '-', 1, cell_gray_style)
                    y_offset_tmp += 1
                    if projects and bu.id in projects:
                        for project in projects[bu.id]:
                            project_value = self._get_cashflow_lines_project(report_lines, key, project)
                            # total_project_value[project]['total_plan_in'] += project_value['plan_in']
                            if total_project_value[project]['total_plan_in'] != 0:
                                write_with_colspan(sheet, x_offset, y_offset_tmp, project_value['plan_in']
                                                   if project_value['plan_in'] != 0 else '-', 1, cell_gray_style)
                                y_offset_tmp += 1
                            if self.report_type == 'reference':
                                ref_lines = self._get_cashflow_lines_project_ref(report_lines, key, project)
                                for ref, value in ref_lines.items():
                                    if total_project_ref['plan_in'][project][ref] != 0:
                                        write_with_colspan(sheet, x_offset, y_offset_tmp, value['plan_in']
                                                           if value['plan_in'] != 0 else '-', 1, cell_gray_style)
                                        y_offset_tmp += 1
                    write_with_colspan(sheet, x_offset, y_offset_tmp,
                                       cell_values['plan_out'] if cell_values['plan_out'] != 0 else '-', 1, cell_gray_style)
                    y_offset_tmp += 1
                    if projects and bu.id in projects:
                        for project in projects[bu.id]:
                            project_value = self._get_cashflow_lines_project(report_lines, key, project)
                            # total_project_value[project]['total_plan_out'] += project_value['plan_out']
                            if total_project_value[project]['total_plan_out'] != 0:
                                write_with_colspan(sheet, x_offset, y_offset_tmp, project_value['plan_out']
                                                   if project_value['plan_out'] != 0 else '-', 1, cell_gray_style)
                                y_offset_tmp += 1
                            if self.report_type == 'reference':
                                ref_lines = self._get_cashflow_lines_project_ref(report_lines, key, project)
                                for ref, value in ref_lines.items():
                                    if total_project_ref['plan_out'][project][ref] != 0:
                                        write_with_colspan(sheet, x_offset, y_offset_tmp, value['plan_out']
                                                           if value['plan_out'] != 0 else '-', 1, cell_gray_style)
                                        y_offset_tmp += 1
                    ...
                    write_with_colspan(sheet, x_offset, y_offset_tmp, total_value['total_actual_in']
                                       if total_value['total_actual_in'] != 0 else '-', 1, cell_style)
                    y_offset_tmp += 1
                    if projects and bu.id in projects:
                        for project in projects[bu.id]:
                            if total_project_value[project]['total_actual_in'] != 0:
                            ...
                            if self.report_type == 'reference':
                                ...
                                for ref, value in ref_lines.items():
                                    if total_project_ref['actual_in'][project][ref] != 0:
                                        write_with_colspan(sheet, x_offset, y_offset_tmp, '', 1, cell_style)
                                        y_offset_tmp += 1
                    write_with_colspan(sheet, x_offset, y_offset_tmp, total_value['total_actual_out']
                                       if total_value['total_actual_out'] != 0 else '-', 1, cell_style)
                    y_offset_tmp += 1
                    if projects and bu.id in projects:
                        for project in projects[bu.id]:
                            if total_project_value[project]['total_actual_out'] != 0:
                    ...
                    y_offset_tmp += 1
                    write_with_colspan(sheet, x_offset, y_offset_tmp, total_value['total_plan_in']
                                       if total_value['total_plan_in'] != 0 else '-', 1, cell_gray_style)
                    y_offset_tmp += 1
                    if projects and bu.id in projects:
                        for project in projects[bu.id]:
                            if total_project_value[project]['total_plan_in'] != 0:
                            ...
                            if self.report_type == 'reference':
                                ...
                                for ref, value in ref_lines.items():
                                    if total_project_ref['plan_in'][project][ref] != 0:
                                        write_with_colspan(sheet, x_offset, y_offset_tmp, '', 1, cell_gray_style)
                                        y_offset_tmp += 1
                    write_with_colspan(sheet, x_offset, y_offset_tmp, total_value['total_plan_out']
                                       if total_value['total_plan_out'] != 0 else '-', 1, cell_gray_style)
                    y_offset_tmp += 1
                    if projects and bu.id in projects:
                        for project in projects[bu.id]:
                            if total_project_value[project]['total_plan_out'] != 0:

...
class FSCombinedPLReportWizard(models.TransientModel):
    _name = 'soa.fs.combined.pl.report'
    _inherit = 'budget.combined.common'
    _description = 'FS Combined (P&L)'
        ...
        for expression in report_line.expression_ids.filtered(lambda x: x.engine == 'account_codes'):
            ...
            for account in account_list:
                ...
                for company_id in company_ids:
                    ...
                    select = f"""
                    SELECT COALESCE(SUM(-account_move_line.balance * currency_table.rate), 0.0) AS sum
                    FROM {from_clause}
        ...
        for company in company_ids:
            ...
            for expression in report_line.expression_ids:
                ...
                for term in terms_to_eval:
                    ...
                    term_value = abs(source_total_amount) * abs(coeff) / 100
                    formula = re.sub(term_replacement_regex % re.escape(term), f'{term_value}', formula)
                try:
                    range_expression_sum += expr_eval(formula)
                except ZeroDivisionError:
    ...
    def _get_lines(self):
        ...
        structure_report = self.env.ref('soa_finance_report.fs_combined_pl', raise_if_not_found=False)
